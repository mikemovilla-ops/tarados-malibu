...
export default async function EstadisticasPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10 space-y-4">
      ...
      {ranking.length === 0 ? (
        <p className="text-chalk/50 text-sm">Todavía no hay datos de partidos jugados.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-chalk/50 text-left border-b border-chalk/10">
                ...
                <th className="py-2 pr-4 font-normal text-center">Asist.</th>
              </tr>
            </thead>
              ...
              {ranking.map((fila) => (
                <tr key={fila.userId}>
                  <td className="py-2 pl-4 text-chalk">
                    {fila.dorsal !== null && <span className="text-malibubright font-display">#{fila.dorsal} </span>}
                    {fila.nombre}
                  </td>
                  <td className="py-2 px-2 text-center text-chalk/70">{fila.partidosJugados}</td>
                  <td className="py-2 px-2 text-center text-malibubright font-display">{fila.goles}</td>
                  <td className="py-2 pr-4 text-center text-chalk/70">{fila.asistencias}</td>
                </tr>
              ))}
            </tbody>
          </table>
